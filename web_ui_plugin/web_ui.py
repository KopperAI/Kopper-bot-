...
def control_process(process_name, action):
    if process_name not in ['telegram', 'discord' , 'rss']:
    ...
    if action == 'start':
        if process_name == 'telegram':
            ...
            if not telegram_token or not telegram_chat_id:
                return jsonify({'status': 'error',
                                'message': 'Veuillez définir le jeton de Télégramme et l’ID de chat dans le panneau de configuration avant de commencer le processus de Télégramme'})
        ...
        #### Début Bot Dicord
            ...
            # Vérifier si discord_token et discord_chat_id sont définis
            discord_token = db.get_parameter('discord_token')
            discord_chat_id = db.get_parameter('discord_chat_id')
            if not discord_token or not discord_chat_id:
                return jsonify({'status': 'error',
                                'message': 'Veuillez définir le jeton de Discord et l’ID de chat dans le panneau de configuration avant de commencer le processus de Discord'})

            # Mettre à jour l’état du processus dans la base de données
            # Le processus du gestionnaire détectera cela et lancera le processus
            db.set_parameter('discord_process_running', 'True')
            logger.info("discord bot process start requested")
            return jsonify({'status': 'success', 'message': 'Discord bot lancement demandé'})
        #### Fin Bot Dicord
        ...
        if process_name == 'telegram':
            ...
            # Mettre à jour l’état du processus dans la base de données
            # Le processus du gestionnaire détectera cela et arrêtera le processus
            db.set_parameter('telegram_process_running', 'False')
            logger.info("Telegram bot arrêt du processus demandé")
        ...
        #### Début Bot Dicord ####
            ...
            # Vérifier l’état actuel
            if db.get_parameter('discord_process_running') != 'True':
                return jsonify({'status': 'warning', 'message': 'Le bot Discord ne fonctionne pas'})

            # Mettre à jour l’état du processus dans la base de données
            # Le processus du gestionnaire détectera cela et arrêtera le processus
            db.set_parameter('discord_process_running', 'False')
            logger.info("Discord bot arrêt du processus demandé")
            return jsonify({'status': 'success', 'message': 'Discord bot arrêt demandé'})
        #### Fin Bot Dicord ####
            ...
            logger.info("RSS feed arrêt du processus demandé")
