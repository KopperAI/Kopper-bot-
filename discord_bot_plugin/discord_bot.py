from discord import Update, InlineKeyboardButton, InlineKeyboardMarkup
from discord.ext import ContextTypes
from discord.error import RetryAfter
...
async def hello(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        await update.message.reply_text(
            f'Bonjour, {update.effective_user.first_name}! Vinted-Bot fonctionne sous la version {ver}.\n')
    except Exception as e:
        logger.error(f"Erreur dans la commande hello: {str(e)}", exc_info=True)
        try:
            await update.message.reply_text('Une erreur s\'est produite. Veuillez réessayer ultérieurement.')
        except:
            pass
...
class LeRobot:
    def __init__(self, queue):
        ...
        from discord.ext import ApplicationBuilder, CommandHandler

        try:

            self.bot = Bot(db.get_parameter("discord_token"))
            self.app = ApplicationBuilder().token(db.get_parameter("discord_token")).build()
            ...
            job_queue.run_once(self.check_discord_queue, when=1)
    ...
    async def send_new_post(self, content, url, text, buy_url=None, buy_text=None):
        try:
            async with self.bot:
                chat_ID = str(db.get_parameter("discord_chat_id"))
    ...
    async def check_discord_queue(self, context: ContextTypes.DEFAULT_TYPE):
        try:
            while 1:
    ...
    async def set_commands(self, context: ContextTypes.DEFAULT_TYPE):
        try:
            await self.bot.set_my_commands([
                ("hello", "Vérifier si le bot est en ligne"),
                ("add_query", "Ajouter un mot-clé au bot"),
                ("remove_query", "Supprimer un mot-clé du bot"),
                ("queries", "Liste tous les mots-clés"),
                ("clear_allowlist", "Effacer la liste des Pays autorisés"),
                ("add_country", "Ajouter un Pays à la liste autorisée"),
                ("remove_country", "Supprimer un Pays de la liste autorisée"),
                ("allowlist", "Énumérer tous les pays dans la liste autorisée")
            ])
            logger.info("Les commandes du Bot ont été définies avec succès")
        except Exception as e:
            logger.error(f"Erreur de configuration des commandes bot: {str(e)}", exc_info=True)
